...
/*   Updated: 2020/02/27 14:17:41 by lsmit         ########   odam.nl         */
/*                                                                            */
/* ************************************************************************** */
...
int		ft_floorcheck(t_vars *vars, char **sub, char *file)
{
	...
	if (sub[1])
	{
		...
		{
			sub = ft_split(sub[1], ',');
			return (ft_floorwithcolour(vars, sub));
		}
		vars->floor->tex = 1;
...
int		ft_ceilingcheck(t_vars *vars, char **sub, char *file)
{
	...
	if (sub[1])
	{
		...
		{
			sub = ft_split(sub[1], ',');
			return (ft_ceilingwithcolour(vars, sub));
		}
		vars->floor->tex2 = 1;
